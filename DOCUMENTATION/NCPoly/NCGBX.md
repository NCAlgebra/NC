...
then produces an summary report on the findings using [NCSortInOrder](#NCSortInOrder).
...
### NCSortInOrder {#NCSortInOrder}

`NCSortInOrder[{poly1, poly2, ...}]` produces an summary report on the list of
polynomials denoted by `{poly1, poly2, ...}`, grouped by the number of unknowns
in each polynomial as determined by the monomial order.

  ...
  Do[ poly=NCE[polynomials[[i]]];
      If[Not[polynomials[[i]]===poly]
        , rule=PolyToRule[poly];
          str=ToStringForTeX[polynomials[[i]]];
          function[rule]=str;
          $NC$NCGBMmaDiagnosticPrint["Polynomial : ",polynomials[[i]]];
          $NC$NCGBMmaDiagnosticPrint["corresponds to rule : ",rule];
      ];
...
Module[{i,j,k,singleRuleList,singleVarList,nonsingleVarList,poly,
  ...
  If[Head[stream]===OutputStream
    , $NC$NCGBMmaDiagnosticPrint["Outputting results to the file ",
                                 stream[[1]]];
    , Print["Outputting results to ",stream];
  ];
...
*)
  ...
  If[Head[stream]===OutputStream
    , $NC$NCGBMmaDiagnosticPrint["Done outputting results to the file ",
                                 stream[[1]]];

...
public:
  typedef GroebnerRule FactType;
  typedef GroebnerRule Fact;
  typedef GBHistory HistoryType;
  typedef GBHistory History;
  typedef GBList<int> InternalPermanentType;
  typedef GBList<Fact> FactListType;
  ...
  // accessor functions
  const Fact & fact(int index) const;
  ...
  { return _facts == fc._facts &&
      _history == fc._history &&
      _extra == fc._extra &&
      _permanent == fc._permanent; };
  bool operator != (const FactControl & fc) const
  { return !((*this)==fc);};
  GBVector<int> PermNumbers() const; 
  inline int numberOfFacts() const;
private: 
   FactListType _facts;
   HistoryListType _history;
   ExtraListType _extra;
   InternalPermanentType _permanent;
...
inline MyOstream & operator << (MyOstream & os, const FactControl & x) { 
  x.PrintArrays(os); 
  return os;
};
...
inline int FactControl::numberOfFacts() const {
  return _facts.size();
};

...
extern IntRep*  negateIt(const IntRep*, IntRep*);
...
// procedural versions of operators
  ...
  friend void     negateIt(const Integer& x, Integer& dest);
...
// coercion & conversion
  ...
  void		  printon(MyOstream& s, int base = 10, int width = 0) const;
  
  friend istream& operator >> (istream& s, Integer& y);
  friend MyOstream& operator << (MyOstream& s, const Integer& y);
...
inline void negateIt(const Integer& x, Integer& dest)
{
  dest.rep = negateIt(x.rep, dest.rep);
}

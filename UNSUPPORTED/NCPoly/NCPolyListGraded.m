...
NCPolyMonomial[s_Rule, n_Integer] :=
  NCPoly[
    {n}, 
    {{NCFromDigits[NCIntegerDigits[s[[1]], n], n], s[[2]]}}
  ];

(* RULE LEX MONOMIAL constructor *)
NCPolyMonomial[s_Rule, {n__Integer}] := 
  NCPoly[
    {n}, 
    {{NCFromDigits[NCIntegerDigits[s[[1]], Flatten[{n}]], {n}], s[[2]]}}
  ];
...
NCPolyDegree[p_NCPoly] := Max @@ Apply[Plus, Part[p, 2, All, 1, 1 ;; -2], 2];
...
NCPolyLeadingTerm[p_NCPoly, 1] := 
  Rule[
    NCFromDigits[NCIntegerDigits[Part[p, 2, -1, 1], p[[1]]], Plus @@ p[[1]]],
    Part[p, 2, -1, 2]
  ];
...
NCPolyLeadingTerm[p_NCPoly, i_Integer] := Quiet[
  Check[ Rule[
           NCFromDigits[
             NCIntegerDigits[Part[p, 2, -i, 1], p[[1]]], Plus @@ p[[1]]
           ]
          ,Part[p, 2, -i, 2]
         ]
        ,$Failed
...
NCPolyDisplay[p_NCPoly, {vars__List}, plus_:List] := 
  plus @@ 
    MapThread[
      Times, 
      { NCPolyGetCoefficients[p],
        Apply[Dot, Map[Part[Flatten[{vars}],#]&, NCPolyGetDigits[p] + 1] /. {} -> 1, 1] }
    ];

NCPolyDisplay[p_NCPoly, vars_List, plus_:List] := 
  NCPolyDisplay[p, {vars}, plus];

NCPolyDisplay[p_?NumericQ, vars_List:{}, plus_:List] := p;
...
NCPolyProduct[r_NCPoly, s_NCPoly] := Module[
  ...
  If [ NCPolyMonomialQ[r] || NCPolyMonomialQ[s],
    ...
    Return[NCPoly[n, MapThread[List,{digits, coeff}]]];

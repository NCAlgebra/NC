...
** DISCLAIMER:
**
**  THIS SOFTWARE IS PROVIDED FREE OF CHARGE 'AS IS' AND JOHAN LINDH MAKES NO
**  EXPRESS OR IMPLIED WARRANTIES WITH RESPECT TO IT. JOHAN LINDH WILL
...
**  - ASSERT() If the expression evaluates to nonzero, execution continues.
**      Otherwise, the ARI handler is called, if present. If not present,
**      the default ARI action is taken (set with mwSetAriAction()).
**      Can be disabled by defining MW_NOASSERT.
**  - VERIFY() works just like ASSERT(), but when compiling without
**      MEMWATCH the macro evaluates to the expression.
**      Can be disabled by defining MW_NOVERIFY.
**  - TRACE() writes some text and data to the log. Use like printf().
...
**      Can be disabled by defining MW_NOTRACE.
*/
...
#undef ASSERT
#endif /* ASSERT */
#define ASSERT(exp)     while(mwAssert((int)(exp),#exp,__FILE__,__LINE__))
#endif /* !MW_NOASSERT */
...
#undef VERIFY
#endif /* VERIFY */
#define VERIFY(exp)     while(mwVerify((int)(exp),#exp,__FILE__,__LINE__))
#endif /* !MW_NOVERIFY */
...
#endif /* TRACE */
/*lint -esym(773,TRACE) */
#define TRACE           /*lint -save -e506 */ 1?(void)0:mwDummyTraceFunction /*lint -restore */
#endif /* !MW_NOTRACE */

// silly modifications by mark stankus
...
template<class T>
DLNode<T>::DLNode(const T& h, DLNode* p, DLNode* n)
        : hd(h) { bk = p; fd = n; }
...
void BaseDLList::error1() const{
  error("front: empty list"); 
};

void BaseDLList::error2() const {
  error("rear: empty list"); 
};

void BaseDLList::error3() const {
  error("null Pix"); 
};
...
int BaseDLList::OK() const {
  if(this==0) { int m=0; m = 1/m;} // done by mxs
  int v = 1;
  if (h != 0) {
    BaseDLNode* t = h;
    long count = 1000; // MXS      // Lots of chances to find h!
    do {
      count--;
      v &= t->bk->fd == t;
      v &= t->fd->bk == t;

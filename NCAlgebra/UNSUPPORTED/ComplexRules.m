   (*Mon, 28 Jul 97 14:28:05 EDT    small correction
Received: by math.uri.edu (5.0/SMI-SVR4) *)
...
WARNING:
	The standard Mma. package ReIm.m sets things so that
	expressions of complex variables "z" are rewritten in terms
	of Re[z], Im[z] (for example).

	Compare this to the output of functions in the package ComplexRules.m,
	where the expressions of complex variables "z" are 
	given in terms of z, Conjugate[z].

	You may load both ReIm.m and ComplexRules.m, but keep in mind
	that the objectives of the packages conflict.  Furthermore,
	programs that need ComplexRules to run most likely will
	not work if ReIm.m has been loaded.
